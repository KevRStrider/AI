...
def fancy_log(mdp, state):
    ...
    print(f"PEEKED VALUE: {mdp.values[peek_index] if peek_index is not None else None}")
...
def fancy_prompt(mdp, state):
    ...
    while True:
        ...
        for i, action in enumerate(valid_actions):
            print(f"{i}. {action}")
        action = input(f"[{valid_actions[0]}] > ").strip() or valid_actions[0]
...
def fancy_report(mdp, episode):
    utility = sum(episode[i] for i in range(2, len(episode), 3))
    print("\nGame finished!")
...
def Qlearning(eps, eta, epsilon=0.1, gamma=1.0):
    ...
    for e in eps:
        ...
        for t in e:
            ...
            Q[(s, a)] += eta * (r + gamma * max([Q.get((s_, act), 0.0) for act in actions]) - Q[(s, a)])
            numUpdates += 1
...
def poleval(mdp, policy, n=100):
    ...
    def Q(state, action):
        return sum(prob * (reward + mdp.discount() * V[newState])
                   for newState, prob, reward in targets(mdp, state, action))
...
def ValueIteration(mdp):
    V = {state: 0 for state in reachable_states(mdp)}
    ...
    for state in reachable_states(mdp):
        print(state, V[state], pi.get(state, ''))
